      ...
      <CardHeader className='flex flex-row items-center w-full justify-between'>
        <div className='flex flex-row gap-4'>
          ...
          <Input
            ...
            placeholder={t('course.table.input')}
          />
        </div>
        <div>
          <Select>
            <SelectTrigger
              aria-label='select filter display variant'
              className='bg-background px-4 py-2 rounded-md hidden sm:flex h-full'>
              <SelectValue
                placeholder={
          ...
          <TableBody>
            {courses.map(course => (
              <TableRow
                ...
                className={cn(
                  'cursor-pointer',
                  selected && course.id === selected.id && 'bg-muted',
                )}
                key={course.id}>
