...
import { useCheckEditQuery, useEditMutation, useGetCourseQuery } from "@/shared/store/services/CourseService";
...
const CoursePage = () => {
    ...
    const { data: isEditable } = useCheckEditQuery(user?.id && id ? { courseId: id, userId: user.id } : skipToken)
                    ...
                    {isEditing
                        ...
                        <>
                            <Textarea
                                ...
                                value={markdown} onChange={onValueChange} />
                            <SpinnerButton disabled={isSavingChanges} onClick={handleSaveChanges}>Save</SpinnerButton>
                        </>
                        :
                        <Markdown className='markdown'>
                            {markdown ??
                                "#### This course is seems empty! :("}
                        </Markdown>
                    }
                    {(isEditing && !markdown) && <Button
                        className="w-fit"
                        variant='ghost'
                        size='sm'
                        onClick={handleSaveChanges}>
                        Add some content</Button>}
                </div>
            </div>
        )
    }
